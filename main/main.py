...
try:
    ...
    from lib.display import get_config_display_free_off
    from lib.display import get_config_display_brightness
...
def clean(tray_icon=None, display_need_off=True):
...
try:
    tray_icon = pystray.Icon(
        ...
        title=_('WeAct Studio System Monitor') + " " + utils.get_version(),
        icon=Image.open(utils.get_root_dir() / "res" / "icons" / "logo.png"),
        menu=pystray.Menu(
            pystray.MenuItem(
                text=_('Configure'),
...
try:
    ...
    start_configure()
    clean_stop(tray_icon, display_need_off=False)

close_windowtoast = utils.WindowToast(_('WeAct Studio System Monitor') + " " + utils.get_version(),_("The icon has been displayed in the tray. Click the icon to open the configuration."),icon=utils.get_root_dir() / "res" / "icons" / "logo.png")
msg_close_handler()
