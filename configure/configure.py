...
if getattr(sys, "frozen", False):
    utils.std_redirect_logfile(executable_name)

import argparse
parser = argparse.ArgumentParser(description="WeActStudio System Monitor Configuration")
parser.add_argument(
    "-s","--start",
    action="store_true",
...
parser.add_argument(
    "-c","--clean",
    action="store_true",
    help="Clean the system monitor",
)

parser.add_argument(
    "-ul","--upgradel",
    action="store_true",
    help="Upgrade the LibreHardwareMonitor",
)
...
if args.clean:
    ...
    if platform.system() == "Windows":
        import ctypes
        from lib import schedule_service

        is_admin = ctypes.windll.shell32.IsUserAnAdmin() != 0
        if is_admin == True:
            schedule_task_name = "WeAct Studio System Monitor"
            schedule_task_path = ('"' + str(utils.get_root_dir() / "configure.exe") + '"')
            schedule_task_arguments = "--start"
            schedule_task_exist, schedule_task_path_get = (schedule_service.task_exists(schedule_task_name))
            if schedule_task_exist == True:
                print(f"delete schedule task {schedule_task_name}: {schedule_task_path_get}")
                schedule_service.delete_task(schedule_task_name)
                result = 0
    sys.exit(result)

# Loading Language
from lib.utils import set_language
_ = set_language(executable_name)

import time
LOCKFILE = executable_dir / str(executable_name+".lock")
...
from lib.sensors.sensors_python import sensors_fans, is_cpu_fan
from lib.lcd.lcd_comm import Orientation
...
class ConfigWindow:
    def __init__(self):
        ...
        self.window.iconphoto(True, tkinter.PhotoImage(file=utils.get_root_dir() / "res" / "icons" / "logo.png"))
        style = ttk.Style()
        ...
        if platform.system() == "Windows" and sys.getwindowsversion().major >= 10:
            ...
            if self.theme_is_dark:
                self.window.tk.call("source", utils.get_root_dir() / "res" / "tk_themes" / "sv_ttk" / "theme" / "dark.tcl")
                style.theme_use("sun-valley-dark")
            else:
                self.window.tk.call("source", utils.get_root_dir() / "res" / "tk_themes" / "sv_ttk" / "theme" / "light.tcl")
        ...
        if platform.system() == "Windows":
            ...
            if is_admin == True:
                ...
                self.schedule_task_path = (
                    '"' + str(utils.get_root_dir() / "configure.exe") + '"'
                )
                self.schedule_task_arguments = "--start"
    ...
    def on_closing_confirm(self):
        ...
        main_window_width = self.window.winfo_width()
        main_window_height = self.window.winfo_height()
        width = self.closing_confirm_frame.winfo_width() + 50
                    ...
                    if self.display_step == 0:
                        ...
                        if self.theme_setting_change == True:
                            try:
                                ...
                                theme_preview = Image.open(utils.get_root_dir() / "res" / "configs" / "no-preview.png")
    ...
    def load_config_values(self):
        try:
            with open(utils.get_root_dir() / "config.yaml", "rt", encoding="utf8") as stream:
    ...
    def on_bootstrap_checkbutton_toggle(self):
        ...
        if v == True:
            ...
            schedule_service.create_login_task(
                self.schedule_task_name, self.schedule_task_path, self.schedule_task_arguments
            )
