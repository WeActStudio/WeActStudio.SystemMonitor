...
import lib.sensors.sensors as sensors
from lib.log import logger

handle = None
Hardware = None
def init(dll_dir: Path):
    global handle, Hardware
    # Import LibreHardwareMonitor dll to Python
    if platform.architecture()[0] == '64bit':
        lhm_dll = str(dll_dir / "64bit" / "LibreHardwareMonitorLib.dll")
    else:
        lhm_dll = str(dll_dir / "32bit" / "LibreHardwareMonitorLib.dll")
    # noinspection PyUnresolvedReferences
    clr.AddReference(lhm_dll)
    # noinspection PyUnresolvedReferences
    if platform.architecture()[0] == '64bit':
        clr.AddReference(str(dll_dir / "64bit" / "HidSharp.dll"))
    else:
        clr.AddReference(str(dll_dir / "32bit" / "HidSharp.dll"))
    # noinspection PyUnresolvedReferences
    ...
    if ctypes.windll.shell32.IsUserAnAdmin() == 0:
        error_str = "Program is not running as administrator. Please run with admin rights or choose another HW_SENSORS option in config.yaml"
        logger.error(
            error_str)
